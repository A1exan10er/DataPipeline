...
#
# Compatible with bash 3.2 (macOS default) — does not use mapfile.
#
...
# Write candidate episode paths (matching desired status) to a temp file,
# one per line — avoids mapfile/process-substitution so it works on bash 3.2.
CANDIDATES_FILE=$(mktemp)
trap 'rm -f "$CANDIDATES_FILE" "$EXISTING_FILE" "$PICKED_FILE"' EXIT
...
sqlite3 "$JOBS_DB" "SELECT mounted_path FROM jobs;" > "$EXISTING_FILE" 2>/dev/null || true
...
while IFS= read -r EP; do
  [ -z "$EP" ] && continue
  if ! grep -qF "$EP" "$EXISTING_FILE"; then
    echo "$EP" >> "$PICKED_FILE"
    picked_count=$((picked_count + 1))
  fi
  if [ "$picked_count" -ge "$COUNT" ]; then
    break
  fi
...
"
  ...
  echo "[$i/$picked_count] $(date '+%H:%M:%S') inserted: $EP_NAME"

  if [ "$i" -lt "$picked_count" ]; then
    sleep "$DELAY"
  fi
done < "$PICKED_FILE"
